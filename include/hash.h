...
class HashMap {
    public:
        ...
        bool remove(K key) {
            ...
            while (cur->next && !equals_func(cur->next->key, key)) 
                cur = cur->next;
    ...
    private:
        ...
        struct HashNode<K, T> *create_node(K key, T value) {
            struct HashNode<K, T> *node = (struct HashNode<K, T>*) kmalloc(sizeof(struct HashNode<K, T>));
            node->key = key;
            node->value = value;
            node->next = nullptr;
            return node;
        ...
        struct HashNode<K, T> *duplicate_node(struct HashNode<K, T> *node) {
            struct HashNode<K, T> *copy = (struct HashNode<K, T>*) kmalloc(sizeof(struct HashNode<K, T>));
            copy->key = node->key;
        ...
        bool insert(K key, T value) {
            ...
            struct HashNode<K, T> *cur = container[index];
            while (cur->next && !equals_func(cur->key, key)) 
                cur = cur->next;
