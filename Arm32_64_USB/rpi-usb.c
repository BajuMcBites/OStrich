...
RESULT HCDStart(void) {
    ...
    // Core USB Controller Configuration
    ...
    *DWC_CORE_AHB = tempAhb;
    ...
    // HNP/SRP Configuration
    coreUsb = *DWC_CORE_CONTROL;
    switch (DWC_CORE_HARDWARE->OperatingMode) {
    ...
    *DWC_CORE_CONTROL = coreUsb;
    ...
    if (DWC_CORE_HARDWARE->HighSpeedPhysical == Ulpi
        && DWC_CORE_HARDWARE->FullSpeedPhysical == Dedicated
        && coreUsb.UlpiFsls) {
        DWC_HOST_CONFIG->ClockRate = Clock48MHz;
    } else {
        DWC_HOST_CONFIG->ClockRate = Clock30_60MHz;
    }
    DWC_HOST_CONFIG->FslsOnly = true;

    // FIFO Configuration
    *DWC_CORE_RECEIVESIZE = ReceiveFifoSize;
    DWC_CORE_NONPERIODICFIFO->Size.Depth = NonPeriodicFifoSize;
    DWC_CORE_NONPERIODICFIFO->Size.StartAddress = ReceiveFifoSize;
    DWC_CORE_PERIODICINFO->HostSize.Depth = PeriodicFifoSize;
    DWC_CORE_PERIODICINFO->HostSize.StartAddress = ReceiveFifoSize + NonPeriodicFifoSize;
    ...
    *DWC_CORE_OTGCONTROL = tempOtgControl;
    ...
    for (int channel = 0; channel < DWC_CORE_HARDWARE->HostChannelCount; channel++) {
        ...
        tempChar = DWC_HOST_CHANNEL[channel].Characteristic;
        tempChar.channel_enable = false;
        tempChar.channel_disable = true;
        tempChar.endpoint_direction = USB_DIRECTION_IN;
        DWC_HOST_CHANNEL[channel].Characteristic = tempChar;
    }

    // Host Port Power and Reset
    struct HostPort tempPort;
...
RESULT EnumerateDevice(struct UsbDevice *device, struct UsbDevice* ParentHub, uint8_t PortNum) {
    RESULT result;
    uint8_t address = device->Pipe0.Number;
    uint32_t transferred;
    struct usb_device_descriptor desc __attribute__((aligned(4))) = {0};
    device->Pipe0.Number = 0;
    device->Pipe0.MaxSize = Bits8;
    struct UsbPipeControl pipectrl = {
    ...
    // Step 1: Read first 8 bytes of Device Descriptor
    result = HCDSumbitControlMessage(
        device->Pipe0,
        pipectrl,
        (uint8_t*)&desc,
        ...
        &(struct UsbDeviceRequest) {
            ...
            .Type = bmREQ_GET_DEVICE_DESCRIPTOR,
            .Value = (uint16_t)USB_DESCRIPTOR_TYPE_DEVICE << 8,
            .Index = 0,
            .Length = 8,
        },
        ControlMessageTimeout,
        &transferred);
    if (result != OK || transferred != 8) {
        dwc_release_channel(pipectrl.Channel);
        return result;
    }
    device->Pipe0.MaxSize = SizeFromNumber(desc.bMaxPacketSize0);
    device->Config.Status = USB_STATUS_DEFAULT;

    // Step 2: Reset Port (if not root hub)
    if (ParentHub != NULL) {
    ...
    // Step 4: Read full Device Descriptor
    result = HCDGetDescriptor(
        ...
        bmREQ_GET_DEVICE_DESCRIPTOR,
        &transferred, true);
    if (result != OK || transferred != sizeof(device->Descriptor)) {
        dwc_release_channel(pipectrl.Channel);
        return result;
    }

    // Step 5: Read Configuration
    struct usb_configuration_descriptor configDesc __attribute__((aligned(4)));
    result = HCDGetDescriptor(device->Pipe0, USB_DESCRIPTOR_TYPE_CONFIGURATION, 0, 0,
        &configDesc, sizeof(configDesc), bmREQ_GET_DEVICE_DESCRIPTOR, &transferred, true);
    if (result != OK || transferred != sizeof(configDesc)) {
        dwc_release_channel(pipectrl.Channel);
        return ErrorDevice;
    }
    device->Config.ConfigStringIndex = configDesc.iConfiguration;

    // Read full configuration data
    uint8_t configNum = configDesc.bConfigurationValue;
    uint8_t configBuffer[1024];
    result = HCDSumbitControlMessage(
        device->Pipe0,
        pipectrl,
        &configBuffer[0],
        ...
        &(struct UsbDeviceRequest) {
            .Request = GetDescriptor,
            .Type = bmREQ_GET_DEVICE_DESCRIPTOR,
            .Value = (uint16_t)USB_DESCRIPTOR_TYPE_CONFIGURATION << 8,
            .Index = 0,
            .Length = configDesc.wTotalLength,
        },
        ControlMessageTimeout,
        &transferred);
    if (result != OK || transferred != configDesc.wTotalLength) {
        dwc_release_channel(pipectrl.Channel);
        return result != OK ? result : ErrorDevice;
    }

    // Parse configuration data for interfaces and endpoints
    uint8_t EndPtCnt = 0;
    uint8_t hidCount = 0;
    uint32_t i = 0;
    while (i < configDesc.wTotalLength - 1) {
        switch (configBuffer[i + 1]) {
        case USB_DESCRIPTOR_TYPE_INTERFACE:
            myMemCopy((uint8_t*)&device->Interfaces[device->MaxInterface],
                &configBuffer[i], sizeof(struct UsbInterfaceDescriptor));
            device->MaxInterface++;
            EndPtCnt = 0;
            break;
        case USB_DESCRIPTOR_TYPE_ENDPOINT:
            myMemCopy((uint8_t*)&device->Endpoints[device->MaxInterface - 1][EndPtCnt],
                &configBuffer[i], sizeof(struct UsbEndpointDescriptor));
            EndPtCnt++;
            break;
        case USB_DESCRIPTOR_TYPE_HID:
            if (hidCount == 0) {
                if ((result = AddHidPayload(device)) != OK) {
                    dwc_release_channel(pipectrl.Channel);
                    return result;
                }
            }
            if (hidCount < MaxHIDPerDevice) {
                myMemCopy((uint8_t*)&device->HidPayload->Descriptor[hidCount],
                    &configBuffer[i], sizeof(struct HidDescriptor));
                device->HidPayload->HIDInterface[hidCount] = device->MaxInterface - 1;
                hidCount++;
            }
            break;
        }
    ...
    device->Config.ConfigIndex = configNum;
    device->Config.Status = USB_STATUS_CONFIGURED;

    // Step 7: Handle special devices
    if (device->Descriptor.bDeviceClass == DeviceClassHub) {
        if ((result = EnumerateHub(device)) != OK) {
            dwc_release_channel(pipectrl.Channel);
            return result;
        }
    } else if (hidCount > 0) {
        device->HidPayload->MaxHID = hidCount;
        if ((result = EnumerateHID(device->Pipe0, device)) != OK) {
            dwc_release_channel(pipectrl.Channel);
            return result;
        }
    }
