...
void *elf_load_file(char* filename) {
	int fd = open(filename, O_RDONLY)
	Elf64_Ehdr *hdr = (Elf64_Ehdr *)read(fd, &hdr, sizeof(ehdr));
...
static int elf_do_reloc(Elf64_Ehdr *hdr, Elf64_Rel *rel, Elf64_Shdr *reltab) {
	...
	switch(ELF32_R_TYPE(rel->r_info)) {
		...
		case R_ARM64_PC32:
			...
			*ref = DO_ARM_PC32(symval, *ref, (int)ref);
			break;
		default:
...
void *load_segment(int fd, Elf64_Phdr *phdr) {
    ...
    read(fd, mapped_memory + page_offset, file_size);
...
static int elf_load_stage3(int fd, Elf64_Ehdr *hdr) {
...
// load
static inline void *elf_load_rel(int fd, Elf64_Ehdr *hdr) {
	int result;
