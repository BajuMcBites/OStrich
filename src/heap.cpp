...
namespace alogx2
{
    ...
    memory_block_t *find(size_t size)
    {
        ...
        while (cur != nullptr)
        {
            ...
            else if (best != nullptr && cur->block_size_alloc < best->block_size_alloc && cur->block_size_alloc >= size)
            {
                best = cur;
            }
            if (best != nullptr)
            {
                if (best->block_size_alloc == size)
                    return best;
            }
            cur = cur->next;
        }
    ...
    memory_block_t *coalesce(memory_block_t *block)
    {
        ...
        while (cur)
        {
            if (cur->next != nullptr && cur->next == (memory_block_t *)((char *)cur + cur->block_size_alloc))
            { // neighbors !
...
void free(void *ptr)
{
    ...
    else if (freeBlock < free_head)
    { // block needs to be head
