...
For the ARMv8 architecture, systems calls fall under a specific type of exception. There are synchronous exceptions, which are generated by certain instructions such as diving by 0 or accessing invalid memory, and there are asynchronous exceptions that are not related to the instruction being excecuted. These could be generated by a timer interrupt for example. In ARMv8, there are four types of exceptions that fall into synchronous or asynchronous.
...
Before moving on what the kernel side needs, it is important to understand how arguements will be passed. ARMv8 passes arguments and return values using registers x0-x8. In the assembly stub, we have done a ***mov*** to get the opcode into the register. The remaining arguments are placed into registers x0-x5 (assuming Linux) when the corresponding function from the header file is called. 
